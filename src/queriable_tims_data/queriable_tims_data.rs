...
pub fn query_multi_group<'a, QD, TL, QF, AE1, AE2, OE, AG, FH, CTX>(
    queriable_data: &'a QD,
    tolerance: &'a TL,
...
where
    AG: Aggregator<Item = AE2, Output = OE, Context = CTX> + Send + Sync,
    QD: QueriableData<QF, AE1, CTX> + ToleranceAdapter<QF, ElutionGroup<FH>>,
    TL: Tolerance,
    OE: Send + Sync,
    FH: Clone + Eq + Serialize + Hash + Send + Sync,
    QF: Send + Sync + ProvidesContext<Context = CTX>,

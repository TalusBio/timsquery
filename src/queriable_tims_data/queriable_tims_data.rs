...
pub fn query_multi_group<'a, QD, TL, QF, AE, OE, AG, FH>(
    queriable_data: &'a QD,
    tolerance: &'a TL,
...
where
    AG: Aggregator<Item = AE, Output = OE> + Send + Sync,
    QD: QueriableData<QF, AE> + ToleranceAdapter<QF, ElutionGroup<FH>>,
    TL: Tolerance,
    OE: Send + Sync,
...
pub fn query_indexed<QD, TA, TL, QF, AE, OE, AG, FH>(
    queriable_data: &QD,
    aggregator_factory: &dyn Fn(u64) -> AG,
    ...
    AE: Send + Sync + Clone + Copy,
{
    ...
    queriable_data.add_query(&prep_query, &mut aggregator);
    aggregator.finalize()
}
